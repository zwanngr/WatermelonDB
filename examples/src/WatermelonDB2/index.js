...
import { DatabaseProvider, useDatabase } from '@react-native-ohos/watermelondb/src/react';
...
const MovieScreen = () => {
  ...
  const seedDemoData = useCallback(async () => {
    console.log('%c watermelondbConsoleLogger moviesCollection:', 'color: #0e93e0;background: #aaefe5;', moviesCollection);
    ...
    async (movie) => {
      await databaseInstance.write(async () => {
        await movie.update((record) => {
          record.title = `${record.title.split(' · ')[0]} · v${Math.floor(
            Math.random() * 10 + 1,
          )}`;
        });
      });
